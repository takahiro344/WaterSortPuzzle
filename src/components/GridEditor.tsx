...
export const GridEditor: React.FC<Props> = ({ image, onBack, onConfirm }) => {
    ...
    const onMove = (e: PointerEvent) => {
      ...
      setGrids(prev => prev.map(grid => {
        ...
        if (corner === "topCenter" || corner === "bottomCenter") {
          ...
          const verticalCorners: (keyof Corners)[] = corner === "topCenter" ? ["tl", "tr"] : ["bl", "br"];
  ...
  useEffect(() => {
    const canvas = canvasRef.current, ctx = canvas?.getContext("2d"); if (!ctx || gridPoints.size === 0) return;
    const map = new Map<string, RGB>();
    for (const grid of grids) {
      const points = gridPoints.get(grid.id); if (!points) continue;
      for (let r = 0; r < points.length; r++) for (let c = 0; c < points[r].length; c++) {
        const p = points[r][c]; map.set(cellKey(grid.id, c, r), sampleColorAt(ctx, p.x, p.y));
      }
    }
  ...
  const handleSolveClick = () => {
    ...
    for (const grid of grids) for (let c = 0; c < grid.cols; c++) {
      const tube: number[] = [];
      for (let r = CAPACITY - 1; r >= 0; r--) { const value = valueByCell.get(`${grid.id}-${c}-${r}`); if (value !== undefined && value !== EMPTY) tube.push(value); }
      tubes.push(tube);
    }
    for (let i = 0; i < emptyTubeCount; i++) tubes.push([]);
    const paletteRgb = palette.slice();
    if (inference.inferredColor !== null && inference.inferredColor >= palette.length) paletteRgb.push(null);
  ...
  const renderHandle = (grid: GridConfig, handle: Handle, p: Point, key: string) => {
    ...
    const fill = preview ? `rgb(${preview.r}, ${preview.g}, ${preview.b})` : "transparent";
    return <circle key={handle} className="corner-handle" cx={p.x} cy={p.y} r={HANDLE_R} style={{ fill }} stroke="#fff" strokeWidth={1.5}
      onPointerDown={e => { e.stopPropagation(); setDragging({ gridId: grid.id, corner: handle, startX: p.x, startY: p.y, startCorners: { ...grid.corners } }); }} />;
  };
  ...
  return (
    <div className="step-panel">
      ...
      <ol className="instructions">
        <li>各グリッドの交点が試験管の色水の中心に来るように、四隅のハンドルを調整してください。</li>
        <li>g+ / g- でグリッドを追加・削除し、c+ / c- で選択中のグリッドの縦線を追加・削除できます。</li>
      ...
      <div className="grid-controls">
        ...
        <button onClick={handleRemoveGrid} disabled={grids.length <= 1}>g-</button><button onClick={handleAddGrid}>g+</button>
        <button onClick={handleRemoveColumn} disabled={selectedGridId === null}>c-</button><button onClick={handleAddColumn} disabled={selectedGridId === null}>c+</button>
        <button onClick={handleResetGrid}>リセット</button>
        <label>空試験管<input type="number" min={0} value={emptyTubeCount} onChange={e => setEmptyTubeCount(Math.max(0, Number(e.target.value) || 0))} /></label>
      </div>
      <div ref={wrapperRef} className="canvas-wrapper" style={{ width: canvasSize.w, height: canvasSize.h }}>
        <canvas ref={canvasRef} />
        {grids.map(grid => {
          const points = gridPoints.get(grid.id); if (!points) return null;
          const selected = grid.id === selectedGridId;
          return <React.Fragment key={grid.id}>
            <svg className="grid-overlay" width={canvasSize.w} height={canvasSize.h} onPointerDown={() => setSelectedGridId(grid.id)}>
              ...
              {points.map((row, r) => row.map((p, c) => {
                const key = cellKey(grid.id, c, r), value = overrides.get(key) ?? AUTO, preview = previewColors.get(key);
                const fill = value === EMPTY ? "transparent" : value === UNKNOWN ? "#fff" : preview ? `rgb(${preview.r}, ${preview.g}, ${preview.b})` : "transparent";
              ...
              {selected && (grid.cols === 1 ? <>{(["topCenter", "bottomCenter"] as Handle[]).map(handle => {
                const row = handle === "topCenter" ? 0 : CAPACITY - 1;
                const p = bilinear(grid.corners, 0.5, row / (CAPACITY - 1));
                return renderHandle(grid, handle, p, cellKey(grid.id, 0, row));
              })}</> : <>{(Object.entries(grid.corners) as [keyof Corners, Point][]).map(([corner, p]) => {
                const col = corner === "tl" || corner === "bl" ? 0 : grid.cols - 1;
                const row = corner === "tl" || corner === "tr" ? 0 : CAPACITY - 1;
                return renderHandle(grid, corner, p, cellKey(grid.id, col, row));
              })}</>)}
            </svg>
          </React.Fragment>;
        })}
