...
function sampleColorAt(ctx: CanvasRenderingContext2D, x: number, y: number): RGB {
  ...
  const sx = Math.min(Math.max(0, Math.round(x - SAMPLE_RADIUS)), Math.max(0, ctx.canvas.width - size));
  const sy = Math.min(Math.max(0, Math.round(y - SAMPLE_RADIUS)), Math.max(0, ctx.canvas.height - size));
...
export const GridEditor: React.FC<Props> = ({ image, onBack, onConfirm }) => {
  ...
  const [dragging, setDragging] = useState<{ gridId: number; corner: Handle; startX: number; startY: number; startCorners: Corners } | null>(null);
  ...
  useEffect(() => {
    ...
    setSelectedGridId(0); setNextGridId(1); setOverrides(new Map()); setPreviewColors(new Map()); setErrorMsg(null);
  }, [image]);
  ...
  const handleSolveClick = () => {
    setErrorMsg(null);
    const canvas = canvasRef.current, ctx = canvas?.getContext("2d"); if (!canvas || !ctx || !grids.length) return;
    const cells: GridCell[] = [];
    for (const grid of grids) {
      const points = gridPoints.get(grid.id); if (!points) continue;
      for (let r = 0; r < CAPACITY; r++) for (let c = 0; c < grid.cols; c++) {
        const pt = points[r][c], key = cellKey(grid.id, c, r);
        cells.push({ gridId: grid.id, col: c, row: r, x: pt.x, y: pt.y, rgb: sampleColorAt(ctx, pt.x, pt.y), value: overrides.get(key) ?? AUTO });
      }
    }
    ...
    if (flatValues.includes(UNKNOWN)) {
      ...
      warnings.push(inference.message); resolvedValues = assignedCells.map(c => c.value === UNKNOWN ? { ...c, value: inference.inferredColor as number } : c);
    }
    ...
    const tubes: number[][] = [];
    for (const grid of grids) for (let c = 0; c < grid.cols; c++) {
      const tube: number[] = []; for (let r = CAPACITY - 1; r >= 0; r--) { const value = valueByCell.get(`${grid.id}-${c}-${r}`); if (value !== undefined && value !== EMPTY) tube.push(value); }
      tubes.push(tube);
    }
  ...
  return (
    <div className="step-panel">
      ...
      <div ref={wrapperRef} className="canvas-wrapper" style={{ width: canvasSize.w, height: canvasSize.h }}><canvas ref={canvasRef} />{grids.map(grid => { const points = gridPoints.get(grid.id); if (!points) return null; const selected = grid.id === selectedGridId; return <React.Fragment key={grid.id}><svg className="grid-overlay" width={canvasSize.w} height={canvasSize.h} onPointerDown={() => setSelectedGridId(grid.id)}>{points.map((row, r) => <line key={`h-${grid.id}-${r}`} className="grid-line" x1={row[0].x} y1={row[0].y} x2={row[row.length - 1].x} y2={row[row.length - 1].y} />)}{points[0]?.map((_, c) => <line key={`v-${grid.id}-${c}`} className="grid-line" x1={points[0][c].x} y1={points[0][c].y} x2={points[points.length - 1][c].x} y2={points[points.length - 1][c].y} />)}{points.map((row, r) => row.map((p, c) => { const key = cellKey(grid.id, c, r), value = overrides.get(key) ?? AUTO, preview = previewColors.get(key); const fill = value === EMPTY ? "transparent" : value === UNKNOWN ? "#fff" : preview ? `rgb(${preview.r}, ${preview.g}, ${preview.b})` : "transparent"; return <circle key={key} cx={p.x} cy={p.y} r={3.5} fill={fill} stroke={value === UNKNOWN ? "#000" : selected ? "#fff" : "#888"} strokeWidth={1} onPointerDown={e => { e.stopPropagation(); cycleOverride(grid.id, c, r); }} />; }))}{selected && (grid.cols === 1 ? (["topCenter", "bottomCenter"] as Handle[]).map(handle => { const row = handle === "topCenter" ? 0 : CAPACITY - 1; return renderHandle(grid, handle, bilinear(grid.corners, 0.5, row / (CAPACITY - 1)), cellKey(grid.id, 0, row)); }) : (Object.entries(grid.corners) as [keyof Corners, Point][]).map(([corner, p]) => { const col = corner === "tl" || corner === "bl" ? 0 : grid.cols - 1; const row = corner === "tl" || corner === "tr" ? 0 : CAPACITY - 1; return renderHandle(grid, corner, p, cellKey(grid.id, col, row)); }))}</svg></React.Fragment>; })}</div>
      {errorMsg && <div className="error-message">{errorMsg}</div>}<div className="step-actions"><button onClick={onBack}>戻る</button><button className="primary" onClick={handleSolveClick}>解く</button></div>
    </div>
  );
};