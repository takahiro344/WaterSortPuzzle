...
export const GridEditor: React.FC<Props> = ({ image, onBack, onConfirm }) => {
  ...
  const handleAddGrid = () => {
    ...
    setSelectedGridId(id); setNextGridId(v => v + 1);
  };
  ...
  const handleRemoveGrid = () => {
    ...
    setOverrides(prev => {
      ...
      for (const [key, value] of prev) if (!key.startsWith(`${removedId}-`)) next.set(key, value);
      return next;
    });
  };
  ...
  const handleResetGrid = () => {
    ...
    setSelectedGridId(0); setNextGridId(1); setOverrides(new Map());
  };

  const handleSolveClick = () => {
    setErrorMsg(null);
    const ctx = canvasRef.current?.getContext("2d"); if (!ctx || !grids.length) return;
    ...
    const flatValues = assignedCells.map(c => c.value), inference = inferUnknownColor(flatValues, CAPACITY);
  ...
  const renderHandle = (grid: GridConfig, handle: Handle, p: Point, key: string) => {
    ...
    const [col, row] = key.split("-").slice(1).map(Number);
    return (
      <circle
        key={handle}
            ...
            <React.Fragment key={grid.id}>
              <svg className="grid-overlay" width={canvasSize.w} height={canvasSize.h} onPointerDown={() => setSelectedGridId(grid.id)}>
                ...
                {selected && (grid.cols === 1 ? (["topCenter", "bottomCenter"] as Handle[]).map(handle => {
                  ...
                  return renderHandle(grid, handle, bilinear(grid.corners, 0.5, row / (CAPACITY - 1)), cellKey(grid.id, 0, row));
